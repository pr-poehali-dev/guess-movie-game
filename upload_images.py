"""
Скрипт для скачивания кадров с TMDB и загрузки в S3.
Запускать локально с включённым VPN!
...
# --- Настройки S3 (вставь свои ключи из настроек проекта) ---
AWS_ACCESS_KEY_ID = "ВСТАВЬ_СЮДА"
AWS_SECRET_ACCESS_KEY = "ВСТАВЬ_СЮДА"
...
def main():
    ...
    for movie in MOVIES:
        ...
        try:
            print(f"[{movie_id}/83] Скачиваю {filename}...", end=" ")
            resp = requests.get(tmdb_url, timeout=15)
    ...
    print(f"\nГотово: {ok}/{len(MOVIES)} загружено успешно")
    print("Результаты сохранены в upload_results.json")
